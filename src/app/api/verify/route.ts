...
export async function POST(req: NextRequest) {
  try {
    ...
    const expectedQuestionId = getNextQuestionId(team.currentQuestionStage);
    if (!expectedQuestionId || expectedQuestionId !== questionId) {
      return NextResponse.json(
        {
          success: false,
          message: "Invalid question for current team stage",
          body: {
            numberOfLives: team.numberOfLives,
            currentQuestionStage: team.currentQuestionStage,
            isDisqualified: team.isDisqualified,
            nextQuestionId: expectedQuestionId,
          },
        },
        {
          status: 409,
    ...
    if (question.answerCode !== answerCode) {
      team.numberOfLives = Math.max(0, team.numberOfLives - 1);
    ...
    team.currentQuestionStage =
      team.currentQuestionStage + 1 > totalQuestions
        ? -1
    ...
    return NextResponse.json(
      {
        ...
        body: {
          ...
          nextQuestionId:
            team.currentQuestionStage === -1
              ? ""
              : team.spotArray[team.currentQuestionStage - 1],
        },
      },
      {
        status: 200,
